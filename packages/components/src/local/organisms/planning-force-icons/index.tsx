...
const PlanningForceIcons: React.FC<PropTypes> = ({ assets, selectedAssets, currentAssets, forceCol, setSelectedAssets, interactive }) => {
  const [clusterGroup, setClusterGroup] = useState<L.MarkerClusterGroup | undefined>(undefined)
  ...
  console.log('planning force icons', forceCol, assets.length, currentAssets, selectedAssets)
  ...
  useEffect(() => {
    ...
    console.log('planning 2:', clustered.length, raw.length)
  ...
  const MarkerCluster = ({ markers }: { markers: AssetRow[] }) => {
    ...
    useEffect(() => {
      if (clusterGroup) {
        console.log('cluster group 1', clusterGroup, typeof clusterGroup)
        clusterGroup.clearLayers()
        console.log('cluster group 2', clusterGroup)
        ...
        const validMarkers = markerList.filter((val: L.Marker | undefined) => val !== undefined) as L.Marker[]
  ...
  const getClusteredMarkerOption = (asset: AssetRow): L.Marker | undefined => {
    const loc: LatLng = asset.position ? asset.position : latLng([0, 0])
    const isSelected = selectedAssets.includes(asset.id)
    const isDestroyed = asset.health && asset.health === 0
...
export default PlanningForceIcons
