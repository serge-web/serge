...
it('configures adjudicate manager correctly', () => {
  ...
  const store2: RouteStore = routeCreateStore(undefined, Phase.Adjudication, forces, 'umpire', 'role-id', platformTypes, false, false)
...
it('configures adjudicate manager correctly with missing current state', () => {
  ...
  if (frigate) {
    ...
    // prepare some routes, and a selected item
    const store2: RouteStore = routeCreateStore(undefined, Phase.Adjudication, forcesCopy, 'umpire', 'role-id', platformTypes, false, false)
    const store: RouteStore = routeSetCurrent(frigateId, store2)
