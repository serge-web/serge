...
const mostColabResponseMock2 = mostColabResponseMockRaw.map((message: MessageCustom): MessageCustom => {
  const collab = message.details.collaboration
  if (collab) {
    if (collab.status === CollaborativeMessageStates.BeingEdited) {
      return {
        ...
        message: {
          ...message.message,
          Reference: message.message.Reference + '2'
        },
        details: {
          ...message.details,
          // @ts-ignore
...
CollaborativeResponseRelease.args = {
  channel: channelCollaborativeResponding,
  messages: [...mostColabResponseMock, ...mostColabResponseMock2],
  isUmpire: true,
