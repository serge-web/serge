...
/* Render component */
export const ChannelRFIMessage: React.FC<Props> = ({ message, borderColor = '#32A3CA', onRead, onChange, forceColor, roleId, roleName, isUmpire, isRFIManager }) => {
  ...
  return (
    <div className={cn(styles['message-list-wrapper'], open && styles.open, message.hasBeenRead && styles.read)} style={{ borderColor }} onClick={handleRead}>
      <Collapsible iconType="channel-icon" onChange={handleCollapse}>
          ...
          <div className={styles.header}>
            ...
            <div className={styles.row}>
              <div>
                ...
                <Badge size="small" label={message.details.from.roleName} customBackgroundColor={forceColor}/>
        ...
        <CollapsibleContent>
          <ChannelRfiMessageDetail isRFIManager={isRFIManager} message={message} onChange={onChange} roleId={roleId} roleName={roleName} isUmpire={isUmpire}/>
        </CollapsibleContent>
      </Collapsible>
    </div>
  )
