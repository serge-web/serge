...
it('declutter routes', () => {
  ...
  // update the marker locaitons

  markers[1].position = store.routes[1].currentLocation
  ...
  const data: DeclutterData = { routes: store, markers: markers }
  const cleaned = routeDeclutter2(data, 0.5)

  // pos2 is the MPA. Nothing else is in it's cell, so
  // it should not get decluttered
  expect(cleaned.routes.routes[2].currentLocation).toEqual(pos2)
  expect(cleaned.routes.routes[1].currentLocation).not.toEqual(pos1)
  expect(cleaned.routes.routes[3].currentLocation).not.toEqual(pos3)
  expect(cleaned.routes.routes[4].currentLocation).not.toEqual(pos4)
})
