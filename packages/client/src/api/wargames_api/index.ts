...
const getWargameDbByName = (name: string): ApiWargameDbObject => {
  const dbObject = wargameDbStore.find(
    (item) => item.name === name || item.name === name + dbSuffix
  )
...
export const listenForWargameChanges = (
  name: string,
...
export const getAllMessages = (dbName: string): Promise<Message[]> => {
  ...
  return db
    .allDocs<Message>({ include_docs: true, descending: true })
...
export const getLatestWargameRevision = (dbName: string): Promise<Wargame> => {
  return getAllMessages(dbName)
    .then((messages) => {
      const latestWargame: MessageInfoType | undefined = messages.find(
...
export const populateWargame = (): Promise<Wargame> => {
  return fetch(serverPath + 'allDbs')
    ...
    .then((dbs: string[]) => {
      ...
      const promises: Promise<Wargame>[] = wargameDbStore.map(
        ({ name, db }) => {
          return getLatestWargameRevision(name)
            .then((res) => ({
              name: db.name,
...
export const createWargame = (): Promise<Wargame> => {
  ...
  return new Promise((resolve, reject) => {
    db.put(settings)
      .then(() => {
        db.get<Promise<Wargame>>(dbDefaultSettings._id)
          .then((res) => {
            resolve(res)
          })
          .catch((err) => {
...
export const checkIfWargameStarted = (dbName: string): Promise<boolean> => {
  return getAllMessages(dbName).then((messages) => {
    const latestWargame = messages.find(
      (message) => message.messageType === INFO_MESSAGE
    )
...
export const exportWargame = (dbPath: string): Promise<Wargame> => {
  ...
  return getAllMessages(dbName).then((messages) => {
    const latestWargame: MessageInfoType = messages.find(
      ({ messageType }) => messageType === INFO_MESSAGE
    ) as MessageInfoType
  ...
  return db
    ...
    .then((res) => {
      ...
      const initiatedWargame: Wargame = {
        ...
        turnEndTime: moment()
          .add(wargame.data.overview.realtimeTurnTime, 'ms')
          .format(),
...
export const createLatestWargameRevision = (
  dbName: string,
  wargame: Wargame
): Promise<Wargame> => {
...
const updateWargame = (
  ...
  return db
    .put({
      ...
      turnEndTime: moment()
        .add(nextWargame.data.overview.realtimeTurnTime, 'ms')
        .format()
    })
    .then(() => {
...
export const updateWargameTitle = (
  dbName: string,
  title: string
): Promise<Wargame> => {
  return getAllWargames().then((games) => {
    if (
      games.some(
        (game) =>
          game && game.title === title && getNameFromPath(game.name) !== dbName
      )
    ) {
...
export const savePlatformTypes = (
  dbName: string,
  data: PlatformType
): Promise<Wargame> => {
  return getLatestWargameRevision(dbName).then((res) => {
...
export const saveChannel = (
  ...
  return getLatestWargameRevision(dbName).then((res) => {
    ...
    const channelNew = channels.every(
      (channel: any) => channel.name !== oldName
    )
...
export const duplicateChannel = (
  ...
  return getLatestWargameRevision(dbName).then((res) => {
    ...
    const channels = updatedData.channels.channels || []
    const channelIndex = channels.findIndex(
      (channel) => channel.uniqid === channelUniqid
    )
    ...
    updatedData.channels.channels = channels
    updatedData.channels.complete =
      calcComplete(channels) && channels.length !== 0
...
export const cleanWargame = (dbPath: string): Promise<WargameRevision[]> => {
  ...
  return db.get(dbDefaultSettings._id).then((res) => {
    const newDb: ApiWargameDb = new PouchDB(
      databasePath + newDbName + dbSuffix
    )
    const wargame = res as Wargame
...
export const duplicateWargame = (
  dbPath: string
): Promise<WargameRevision[]> => {
  const dbName = getNameFromPath(dbPath)
  const { db } = getWargameDbByName(dbName)
  const uniqId = uniqid.time()
  const newDbName = `wargame-${uniqId}`
  ...
  return db.replicate
    ...
    .then((res) => {
      return updateWargame(
        { ...res, _id: dbDefaultSettings._id, name: newDbName },
        newDbName,
        false
      )
    })
    ...
    .then((res) => {
      switch (res.phase) {
        ...
        case ADJUDICATION_PHASE:
          ...
          res.gameTurn += 1
          res.data.overview.gameDate = moment(res.data.overview.gameDate)
            .add(res.data.overview.gameTurnTime, 'ms')
            .format('YYYY-MM-DDTHH:mm')
          res.turnEndTime = moment()
            .add(res.data.overview.realtimeTurnTime, 'ms')
            .format()
          break
      }
...
export const postFeedback = (
  dbName: string,
  fromDetails: MessageDetailsFrom,
  message: string
  ...
  return new Promise((resolve, reject) => {
    getLatestWargameRevision(dbName)
      .then((res) => {
        ...
        // @ts-ignore
        res.data.forces.forces = handleForceDelta(
          message,
          details,
          res.data.forces.forces
        )
