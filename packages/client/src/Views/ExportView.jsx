      ...
      tabs: [
        {
          ...
          url: EXPORT_ROUTE + EXPORT_MESSAGES_SUBROUTE,
          urlalt: EXPORT_ROUTE
        },
        {
          title: 'Export Forces',
          url: EXPORT_ROUTE + EXPORT_FORCES_SUBROUTE,
          urlalt: null
        }
      ]
    }
      ...
      <div className="view-wrapper view-wrapper-gamesetup">
        <ul className="tab-nav">
          {this.state.tabs.map((tab, key) => (
            <li
              key={key}
              className={this.checkTab(tab)}
              onClick={() => {
                this.props.setTab(tab.url)
              }}
            >
              {tab.title}
            </li>
        ...
        <div id="sidebar">
          <Link href={ADMIN_ROUTE} id="home-btn">
            <FontAwesomeIcon icon={faArrowLeft} size="2x" />
          </Link>
        </div>
        <div className="export-container">{this.props.children}</div>
      </div>
    )
  }
}
...
const mapStateToProps = ({ currentViewURI, wargame }) => ({
  currentViewURI,
  wargame
})
...
const mapDispatchToProps = (dispatch) => ({
  setTab: (tab) => {
    dispatch(setCurrentViewFromURI(tab))
  }
})
