...
const factory = (state: PlayerUi): Factory => {
  ...
  const mappingConstraints: MappingConstraints =
    state.mappingConstaints !== undefined
      ? state.mappingConstaints
      : {
          bounds: [
            [14.194809302, 42.3558566271],
            [12.401259302, 43.7417816271]
          ],
          tileDiameterMins: 5,
          tileLayer: {
  ...
  return (node: TabNode): React.ReactNode => {
    ...
    if (channelsArray.length === 1) {
      const isOnlyMap = channelsArray.find(
        (entry) => entry[1].name.toLowerCase() === CHANNEL_MAPPING
      )
      if (isOnlyMap) {
      ...
      const channelDefinition = state.allChannels.find(
        (channel) => channel.name === node.getName()
      )
      if (channelName === CHANNEL_MAPPING) {
        ...
        const roles =
          (state.selectedForce &&
            state.selectedForce.roles.map((role) => role.name)) ||
          []
        return (
          <RfiStatusBoard
            rfiData={{
