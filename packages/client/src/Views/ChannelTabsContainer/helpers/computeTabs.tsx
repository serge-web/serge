...
const renameTabs = (nameChanges: TabMapped[], model: ModelLoc): void => {
  nameChanges.forEach((channel) => {
    model.doAction(
      FlexLayout.Actions.updateNodeAttributes(channel.id, {
        name: channel.name
      })
    )
  })
...
const computeTabs = (state: PlayerUi, modelOrig: Model): void => {
  ...
  const modelTabs = getModelTabs(model)
  const newChannels = _.differenceBy(
    channelNames,
    modelTabs,
    (channel) => channel.id
  )
  const channelsToRemove = _.differenceBy(
    modelTabs,
    channelNames,
    (channel) => channel.id
  )
  const matchingChannels = _.intersectionBy(
    channelNames,
    modelTabs,
    (item) => item.id
  )
  const channelsToRename: TabMapped[] = _.differenceBy(
    matchingChannels,
    modelTabs,
    (item) => item.name
  )
