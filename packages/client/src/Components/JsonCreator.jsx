...
class JsonCreator extends Component {
  ...
  componentWillUpdate (nextProps) {
    if (
      this.editor &&
      nextProps.umpireMenu.selectedSchemaID !==
        this.props.umpireMenu.selectedSchemaID
    ) {
      this.editor.destroy()
      this.editor = null
    }
    ...
    if (
      ...
      this.editor = new JSONEditor(this.editorPreviewRef.current, {
        schema: nextProps.messageTypes.messages.find(
          (mes) => mes._id === nextProps.umpireMenu.selectedSchemaID
        ).details,
        theme: 'bootstrap4'
      })
    }
    ...
    if (
      ...
      const data = nextProps.messages.messages.find(function (mes) {
        return (
          mes._id
            .toLowerCase()
            .indexOf(nextProps.messages.messagePreviewId.toLowerCase()) > -1
        )
      })
  ...
  saveMessage = () => {
    const schema = this.props.messageTypes.messages.find(
      (mes) => mes._id === this.props.umpireMenu.selectedSchemaID
    ).details

    this.props.dispatch(
      createMessage(this.editor.getValue(), {
        ...schema,
        _id: this.props.umpireMenu.selectedSchemaID
      })
    )
  };
    ...
    const SaveMessageButton = () =>
      this.editor
        ...
        <div className="button-wrap">
          {!this.props.disabled
            ? (
            <Button color="secondary" onClick={this.saveMessage} icon="save">
              Save Message
            </Button>
              )
            : null}
