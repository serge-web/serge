...
class AddRoleModal extends Component {
  constructor (props) {
    ...
    this.state = {
      roleName: this.props.currentModal.data
        ? this.props.currentModal.data.name
        : '',
      rolePassword: this.props.currentModal.data
        ? this.props.currentModal.data.password
        : `p${uniqId.time()}`,
      isObserver: this.props.currentModal.data
        ? this.props.currentModal.data.isObserver
        : false,
      canSubmitPlans: this.props.currentModal.data
        ? this.props.currentModal.data.canSubmitPlans
        : false,
      isInsightViewer: this.props.currentModal.data
        ? this.props.currentModal.data.isInsightViewer
        : false,
      isRFIManager: this.props.currentModal.data
        ? this.props.currentModal.data.isRFIManager
        : false
    }
  }
  ...
  setNewRoleName = (value) => {
    ...
    const selectedForce = this.props.wargame.data.forces.selectedForce.name
    this.setState({
      roleName: value,
      sameName: this.props.wargame.data[curTab].forces
        .find((force) => force.name === selectedForce)
        .roles.some((role) => role.name === value)
    })
  };
  ...
  setNewRolePassword = (value) => {
    ...
    this.setState({
      rolePassword: value,
      samePassword: this.props.wargame.data[curTab].forces
        .find((force) => force.name === selectedForce)
        .roles.some((role) => role.password === value)
    })
  };
  ...
  addRole = () => {
    ...
    const newRole = {
      ...
      isGameControl: this.props.currentModal.data
        ? this.props.currentModal.data.isGameControl
        : false
    }

    if (this.props.currentModal.data) {
      this.props.dispatch(
        updateRole(selectedForce, this.props.currentModal.data.name, newRole)
      )
    } else {
  ...
  render () {
    ...
    const disabled =
      this.state.roleName.length < 1 ||
      this.state.sameName ||
      this.state.samePassword ||
      this.state.rolePassword.length > 30
    const selectedForce = this.props.wargame.data.forces.selectedForce.uniqid
    ...
    return (
      <ModalWrapper>
        <div className="display-text-wrapper">
          <h3>Add a role</h3>
          {this.state.sameName && (
            <p className="notification">Name already exists</p>
          )}
          {this.state.samePassword && (
            <p className="notification">Password already exists</p>
          )}
          {this.state.rolePassword.length > 30 && (
            <p className="notification">Password limit is 30 chars.</p>
          )}
