...
class ColorPickerModal extends Component<{ dispatch: Dispatch, currentModal: ModalActionInterface, wargame: Wargame }> {
  ...
  render() {
    ...
    let selectedForceId = this.props.wargame.data.forces.selectedForce;
    let currentForce = this.props.wargame.data.forces.forces.find((force: ForceData) => force.uniqid === selectedForceId);
