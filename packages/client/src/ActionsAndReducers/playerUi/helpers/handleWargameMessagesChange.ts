 ...
 */
export const handleSetLatestWargameMessage = (
  ...
  const res: SetWargameMessage = handleChannelUpdates(
    payload,
    newState.channels,
    newState.chatChannel,
    newState.rfiMessages,
    newState.nextMsgReference,
    newState.selectedForce,
    newState.allChannels,
    newState.selectedRole,
    newState.isObserver,
    newState.allTemplates,
    newState.allForces
  )
  return res
}
 ...
 */
export const handleSetAllMessages = (
  payload: Array<MessageCustom | MessageInfoType>,
  newState: PlayerUi
): SetWargameMessage => {
  const res: SetWargameMessage = handleAllInitialChannelMessages(
    payload,
    newState.currentWargame,
    newState.selectedForce,
    newState.selectedRole,
    newState.allChannels,
    newState.allForces,
...
export const openMessage = (
  channel: string,
  payloadMessage: MessageChannel,
  newState: PlayerUi
): ChannelUI => {
  // mutating `messages` array - copyState at top of switch
  const channelMessages: Array<MessageChannel> =
    newState.channels[channel].messages || []
    ...
    for (const i in channelMessages) {
      ...
      if (res.changed) {
        ...
        expiredStorage.setItem(
          `${newState.currentWargame}-${newState.selectedForce}-${newState.selectedRole}${payloadMessage._id}`,
          'read',
  ...
  const unreadMessageCount = channelMessages.filter((message) => {
    return (
      ...
      expiredStorage.getItem(
        `${newState.currentWargame}-${newState.selectedForce}-${newState.selectedRole}${message._id}`
      ) === null
    )
  }).length
...
export const markAllAsRead = (
  ...
  const channelMessages: MessageChannel[] = (
    newState.channels[channel].messages || []
  ).map((message) => {
    if (message._id) {
      message.hasBeenRead = true
