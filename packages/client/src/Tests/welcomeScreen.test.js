...
describe('Welcome screen edit page', () => {
  ...
  test('Go to welcome screen page', async () => {
    ...
    let title = await page.evaluate(() => document.querySelector('h1').innerText);
  ...
  test('should navigate away from and back to check data saved', async () => {
    ...
    let title = await page.$eval('input[title="Click to edit"]', el => el.value);

    expect(title).toEqual('end to end');

    let description = await page.$eval('.description .paragraph', el => el.innerText);
