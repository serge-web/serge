...
describe('Forces tab', () => {
  ...
  test('Should show name already exists warning', async () => {
    ...
    let nameexistswarning = await page.evaluate(() => document.querySelector('p.notification').innerText);
  ...
  test('Should update the white overview & objectives', async () => {
    ...
    let objectivesText = await page.evaluate(() => document.querySelector('textarea[name="objectives"]').value);
  ...
  test('Should create a new force and be able to delete it', async () => {
    ...
    let forcelist2nd = await page.evaluate(() => {
      return [...document.querySelectorAll('.list-title')].map((force) => force.innerText);
    });
...
describe('Channels tab', () => {
  ...
  test('Should duplicate a channel', async () => {
    ...
    let channelList = await page.evaluate(() => [...document.querySelectorAll('.list-title')].map((channel) => channel.innerText));
  ...
  test('Should delete second row in table', async () => {
    ...
    let deleteIcons = await page.$$('svg[data-icon="trash"]');
